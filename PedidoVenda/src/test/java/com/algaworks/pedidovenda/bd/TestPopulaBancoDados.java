...
public class TestPopulaBancoDados {
    ...
    @Test
    public void testPopulaGrupo() {
	this.produceEntityManager();

	this.em.getTransaction().begin();

	this.em.createNativeQuery("insert into grupo (nome, descricao) values('ADMINISTRADORES', 'Grupo com todas as permissões')").executeUpdate();
	this.em.createNativeQuery("insert into grupo (nome, descricao) values('AUXILIARES', 'Grupo com pouca permissão')").executeUpdate();
	this.em.createNativeQuery("insert into grupo (nome, descricao) values('VENDEDORES', 'Grupo para usuários do sistema')").executeUpdate();
